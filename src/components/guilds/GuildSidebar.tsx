...
function GuildDropdownButton(props: GuildDropdownButtonProps) {
  ...
  return (
    <li
      class={`mx-1.5 rounded-lg group/gdb ${groupHoverClass} transition-all duration-300`}
    >
...
function Channel(props: ChannelProps) {
  ...
  const mentionCount = createMemo(() =>
    cache.countGuildMentionsIn(guildId(), props.channel.id),
  );
  const permissions = createMemo(() =>
    cache.getClientPermissions(guildId(), props.channel.id),
  );
  ...
  const markRead = async () => {
    ...
    if (lastMessageId) {
      await api.request(
        "PUT",
        `/channels/${props.channel.id}/ack/${lastMessageId}`,
      );
    }
  };
  ...
  return (
      ...
      onContextMenu={contextMenu.getHandler(
        <ContextMenu>
          ...
          <ContextMenuButton
            ...
            onClick={() =>
              window.navigator.clipboard.writeText(props.channel.id.toString())
            }
          />
    ...
    >
      <Icon
        ...
        classList={{
          [isUnread() || active() ? "fill-fg/100" : "fill-fg/60"]: true,
        }}
      />
      <span class="flex justify-between items-center flex-grow">
        ...
        <span class="flex items-center gap-x-1">
          <Show when={permissions().has("CREATE_INVITES") && hovered()}>
            <button
              onClick={(e) => {
                e.preventDefault();
                showModal(ModalId.CreateInvite, { channel: props.channel });
              }}
            >
              <Icon
                icon={UserPlus}
                class="w-3 h-3 fill-fg/50 transition hover:fill-fg/100"
                tooltip={{ content: "Create Invite", placement: "right" }}
              />
            </button>
          </Show>
          <Show when={permissions().has("MODIFY_CHANNELS") && hovered()}>
            <A href={settings()}>
              <Icon
                icon={Gear}
                class="w-3 h-3 fill-fg/50 transition hover:fill-fg/100"
...
function GuildMajorLink(props: {
  ...
  return (
    <A
      href={props.href}
      class="rounded-xl flex items-center p-1 gap-x-2 group transition hover:bg-3"
    >
      <div
      ...
      <span
        class="font-title transition"
        classList={{ [props.active ? "text-fg/100" : "text-fg/70"]: true }}
      >
        {props.label}
      </span>
    </A>
...
export default function GuildSidebar() {
  ...
  const channelId = createMemo(
    () => params.channelId && BigInt(params.channelId),
  );
  const contextMenu = useContextMenu()!;
  const { showModal } = useModal();
  ...
  const isOwner = createMemo(
    () => guild().owner_id === api.cache?.clientUser?.id,
  );
  ...
  const guildPermissions = createMemo(() =>
    api.cache?.getClientPermissions(guildId()),
  );
  ...
  const channels = createMemo(() => {
    const channels = api.cache?.guildChannelReactor
      ...
      ?.filter(
        (c) =>
          c &&
          api.cache?.getClientPermissions(guildId(), c.id).has("VIEW_CHANNEL"),
      );
    if (!channels) return;

    const groups = new Map<
      bigint | null,
      { category: GuildChannel; children: GuildChannel[] }
    >([
    ...
    channels.forEach((c) =>
      groups.get(c.parent_id ? BigInt(c.parent_id) : null)?.children.push(c),
    );
  ...
  const RenderChannel = (props: {
    ...
    <Show
      when={props.channel.type === "category"}
      fallback={
        <Show
          when={
            !props.collapsed || api.cache?.isChannelUnread(props.channel.id)
          }
        >
    ...
    >
      <RenderCategory
        id={props.channel.id}
        group={channels()?.get(props.channel.id)!}
      />
    </Show>
  );
  ...
  const RenderCategory = (props: {
    id: bigint;
    group: { category: GuildChannel; children: GuildChannel[] };
  }) => {
    return (
      <Show
        when={
          ...
          (props.id
            ? api.cache!.getClientPermissions(guildId(), props.id)
            : guildPermissions()
          )?.has("MANAGE_CHANNELS")
        }
      >
          ...
          onContextMenu={contextMenu.getHandler(
            <ContextMenu>
              <ContextMenuButton
                ...
                onClick={() =>
                  window.navigator.clipboard.writeText(props.id.toString())
                }
              />
              <Show when={guildPermissions()?.has("MANAGE_CHANNELS")}>
                <DangerContextMenuButton
                  ...
                  onClick={() =>
                    showModal(ModalId.DeleteChannel, props.group.category)
                  }
                />
              </Show>
            </ContextMenu>,
        ...
        >
          <button
            ...
            onClick={() =>
              collapsed.has(props.id)
                ? collapsed.delete(props.id)
                : collapsed.add(props.id)
            }
          >
            <Icon
          ...
          <Show when={guildPermissions()?.has("MANAGE_CHANNELS")}>
            <button
              ...
              onClick={() => {
                showModal(ModalId.CreateChannel, {
                  guildId: guildId(),
                  parentId: props.id,
                });
              }}
        ...
        >
          {(channel) => (
            <RenderChannel
              channel={channel}
              collapsed={collapsed.has(props.id)}
            />
          )}
        </For>
      </Show>
  ...
  return (
        ...
        <ContextMenu>
          ...
          <Show when={guildPermissions()?.has("MANAGE_CHANNELS")}>
            <ContextMenuButton
              ...
              onClick={() =>
                showModal(ModalId.CreateChannel, { guildId: guildId() })
              }
            />
            <ContextMenuButton
              icon={FolderPlus}
    ...
    >
      ...
      >
        ...
        >
          <span class="inline-flex items-center font-title font-bold text-base truncate min-w-0 pr-2">
            {guild().name}
            <Show when={guildId().toString() === "1263260997779456"}>
              <span
                class="ml-2 px-1.5 py-0.5 text-xs rounded bg-accent text-fg font-semibold"
        ...
        {guild().description && (
          <div class="card-body px-3 pt-1 pb-3">
            <p class="text-xs text-fg/50 truncate min-w-0">
              {guild().description}
            </p>
          </div>
        )}
