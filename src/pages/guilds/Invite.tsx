...
const Fallback = () => (
  <p class="text-gray-500 animate-pulse text-2xl font-bold font-title">Loading...</p>
)
...
export default function InviteScreen() {
  ...
  return (
    <Layout title={invite() ? `Accept server invite?` : 'Resolving invite...'}>
      <div class="flex items-center justify-center w-full h-full bg-cover" ref={backgroundRef!}>
        <Show when={invite()} keyed={false} fallback={<Fallback />}>
          <div
            class="flex flex-col bg-gray-900/80 px-8 rounded-xl w-[clamp(512px,70%,768px)]
              mobile:w-[90%] backdrop-blur"
          >
            <p class="py-6 flex items-center justify-center text-sm text-base-content/40 font-title font-bold">
              <Icon icon={UserPlus} class="w-5 h-5 mr-2 fill-base-content/40" />
            ...
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                <p class="flex items-center gap-x-1 text-base-content/50 text-sm">
                  <Icon icon={Users} class="w-4 h-4 fill-base-content/50" />
                  <span class="mr-2">{invite()!.guild!.member_count!.total.toLocaleString()}</span>

                  <Show when={owner()} keyed={false}>
                    <Icon icon={UserTie} class="w-4 h-4 fill-base-content/50" />
                    <span>@{owner()!.username}</span>
                  </Show>
                </p>
                <p class="text-gray-500">{invite()!.guild!.description}</p>
              </div>
            </div>
